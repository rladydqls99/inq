...
describe("deck and card routes", () => {
  ...
  it("rejects non-string deck titles", async () => {
    const { prisma, cleanup } = await createTestPrisma();
  ...
  it("returns not found when listing cards for a missing deck", async () => {
    const { prisma, cleanup } = await createTestPrisma();
    ...
    try {
      ...
      const response = await app.request("/api/decks/missing-deck/cards", {
        headers: { cookie },
      });
