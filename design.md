...
**The Immediate Clarity Rule.** 각 화면은 제목, 현재 상태, 다음 행동을 위에서부터 한 번에 읽을 수 있게 배치한다. 주요 행동은 짧고 결과가 예측되는 동사형 레이블을 사용하며, 삭제·수정 같은 관리 행동은 메뉴 또는 보조 버튼으로 낮춘다. 덱과 챌린지 목록은 카드 더미가 아니라 구분선과 여백을 사용한 리스트 행으로 구성한다.
...
- **Compact Primary:** 목록·상세의 시작 행동은 최소 높이 44px, 좌우 여백 14–16px로 유지한다. 학습 중 엄지 영역의 다음 행동만 48px 이상으로 확장한다.
...
- **Text Button Rule:** 텍스트가 포함된 버튼에는 아이콘을 넣지 않는다. 아이콘은 레이블이 없는 아이콘 전용 버튼에만 사용하고, 접근 가능한 이름을 제공한다.
